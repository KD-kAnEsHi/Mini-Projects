...
if __name__ == '__main__':
    ...
    for iter in [10, 100,1000,10000]:
        for a in [0.01,0.1, 0.33]:
            ...
            lr.fit(Xtrn, ytrn, lr=a, iters=iter, recompute=True) #train the model
    ...
    Xtst = np.hstack((np.ones((Xtst.shape[0], 1)), Xtst))

for a in [0.01, 0.1, 0.33]:
    train_losses = []
    test_losses = []
    iterations = list(range(100, 1100, 100))  # Every 100 iterations
    lr.fit(Xtrn, ytrn, lr=a, iters=1)  # Initialize model

    for i in range(10):  # Train in steps of 100 iterations
        losses = lr.fit(Xtrn, ytrn, lr=a, iters=100, recompute=False) 
